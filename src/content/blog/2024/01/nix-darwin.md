...
[^1]:
    Since this series will focus on system configuration rather than development environments,
...
[^2]:
    If you're on a brand new machine, the first time you run `git` in the terminal you should be
...
[^4]:
    While it's not necessary to fully understand this right now, the `configuration` value that
...
[^3]:
    While we won't be installing any App Store apps in this post, you can check out the
...
"uninstall"`. Your homebrew config in `nix-darwin` is now _declarative_: only the packages specified
